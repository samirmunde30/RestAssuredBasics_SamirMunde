...
public class DemoResponseSpec {
	...
	public void createResponseSpec() {
		...
		responseBuilder.expectStatusCode(200).expectStatusLine("HTTP/1.1 200 OK").expectContentType(ContentType.JSON)
	...
	public void getAllBookingIds() {
		RestAssured.given().baseUri("https://restful-booker.herokuapp.com/booking").when().get().then()
				.spec(responseSpec).body("size()", Matchers.greaterThan(0));
	...
	public void getBookingByFirstName() {
		RestAssured.given().baseUri("https://restful-booker.herokuapp.com/booking?firstname=sally").when().get().then()
				.spec(responseSpec);
		/*
		 * .statusCode(200) .statusLine("HTTP/1.1 200 OK")
		 * .contentType(ContentType.JSON) .time(Matchers.lessThan(3000L));
