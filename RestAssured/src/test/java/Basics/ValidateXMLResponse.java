...
public class ValidateXMLResponse {
	...
	public void GetTravellersData() {
		...
		// Approach 1
	//	 response.then().body("TravelerinformationResponse.travelers.Travelerinformation[2].name",
	//	 Matchers.equalTo("vano"));

		// Approach 2
		XmlPath objXmlPath = new XmlPath(response.asString());

		String travellerName = objXmlPath.get("TravelerinformationResponse.travelers.Travelerinformation[0].name")
				.toString();
	...
	public void AddPet() {
		...
		// verify status code
		Assert.assertEquals(response.statusCode()/* actual status code */, /* expected status code */200,
